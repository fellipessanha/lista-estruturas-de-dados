...
public:
    ...
    explicit Elemento<T>(T value, Elemento<T>* next=0) : value(value), next(next) {}
};
...
void letraA() {
    static_assert(
        requires(exercicio::DequeEncadeado<char> a, char t){
...
void letraB() {
    static_assert(
        requires(exercicio::PilhaDeque<char> a, char t){
...
void letraC() {
    static_assert(
        requires(exercicio::FilaDeque<char> a, char t){
